class Instagram::MessageText < Instagram::BaseMessageText
  ...
  def ensure_contact(ig_scope_id)
    ...
    find_or_create_contact_inbox(result) if result.present?
  end
  ...
  def fetch_instagram_user(ig_scope_id)
    ...
    url = "#{base_uri}/#{ig_scope_id}?fields=#{fields}&access_token=#{@inbox.channelable.access_token}"
  ...
  def handle_error_response(response, ig_scope_id)
    ...
    exception = StandardError.new("[InstagramUserFetchError]: account_id #{@inbox.account_id} inbox_id #{@inbox.id} ig_scope_id #{ig_scope_id} (Error message: #{error_message})(Code: #{error_code})")
    raise exception
  ...
  def create_message
    ...
    Instagram::Messages::MessageBuilder.new(@messaging, @inbox, outgoing_echo: agent_message_via_echo?).perform
  end
